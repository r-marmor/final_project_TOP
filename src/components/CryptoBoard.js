...
export default function CryptoBoard() {
    ...
    return (
        <section className="flex flex-col items-center bg-gray-100 h-3/4 py-20 w-full">
            <div className="container mx-auto w-2/3 flex gap-3 mb-5 text-sm">
                <p className="h-10 pl-2 w-1/4 border-gray-500 border-b text-gray-400">Assets</p>
                <p className="h-10 text-right w-1/4 border-gray-500 border-b text-gray-400 hidden sm:block">Price</p>
                ...
                <div key={index} className="container mx-auto w-2/3 flex gap-3 mb-2 text-sm">
                    <p className="flex pl-2 items-center gap-2 h-10 w-1/4 border-gray-500 border-b text-gray-400"><img width="25" src={crypto.logo} alt="currencyLogo"></img><span className="font-bold text-black">{crypto.name}</span><span className="hidden sm:block">{crypto.currency}</span></p>
                    <p className="h-10 text-right w-1/4 border-gray-500 border-b hidden sm:block">${crypto.price}</p>
